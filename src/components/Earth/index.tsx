...
import {WebGLRenderer, RenderState, onWheel, onMouseMove} from "./webgl_earth"
...
export const Earth: FunctionComponent<{
    ...
    fullscreen: boolean,
    ifControls: boolean
}> = ({className, setFullscreen, fullscreen, ifControls}) => {
    const earthCanvasRef = useRef<HTMLCanvasElement>(null);
    const canvas = earthCanvasRef.current

    useEffect(() => {
        if (!canvas) return
        const nameElement = document.querySelector("#name") as HTMLCanvasElement;
        const timeElement = document.querySelector("#time") as HTMLCanvasElement;
        const nameNode = document.createTextNode("");
        const timeNode = document.createTextNode("");
        nameElement?.appendChild(nameNode);
        timeElement?.appendChild(timeNode);

        const overlay = document.querySelector("#nameOverlay") as HTMLCanvasElement;

        const renderState = new RenderState();
        const renderer = new WebGLRenderer(canvas, renderState);
        const res = "/src/components/Earth/webgl_earth/build/res/";
        ...
        renderer.loadRegions(regionsJSON.map((points) => {
            return {lines: points, ID: 0};
        }));
    ...
    return (
        <div className={`${className}`}>
            <canvas id="glCanvas" ref={earthCanvasRef} className={`w-full h-full rounded-3xl overflow-hidden`}></canvas>
            <div id="timeOverlay" className="absolute top-0 left-0 text-white">
                {ifControls && <>
                <div
                    className={`relative left-2 top-2 p-2 rounded-lg flex flex-col space-y-1 bg-black/30 backdrop-blur-lg transition-all`}>
